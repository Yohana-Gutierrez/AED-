...
class Array(object):
    ...
    def removeDupes(self): # eliminar duplicados
        for x in self.__a:
            # verificar si el es> 1 (elemento repetitivo)
            if self.__a.count(x) > 1:
                # si es verdadero, elimina la primera aparición
                self.__a.remove(x)
        return self.__a
        ...
        """rep = item;
        for j in range(self.__nItems): # de un artículo
            if self.__a[j] == rep: # Elemento encontrado
                self.__nItems -= 1 # Uno menos al final
                for k in range(j, self.__nItems): # Mover elementos de
                    self.__a[k] = self.__a[k+1] # justo sobre 1"""
